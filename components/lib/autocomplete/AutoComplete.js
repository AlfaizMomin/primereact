...
export const AutoComplete = (props) => {
    ...
    const ariaSelected = useRef(false);
    const documentClickListener = useRef(null);
    ...
    const selectItem = (event, option, preventInputFocus) => {
        if (props.multiple) {
            ...
            if (!isSelected(option)) {
                let newValue = props.value ? [...props.value, option] : [option];
    ...
    const removeItem = (event, index) => {
        let removedValue = props.value[index];
        let newValue = props.value.filter((_, i) => (index !== i));
        updateModel(event, newValue);
            ...
            switch (event.which) {
                ...
                case 8:
                    if (props.value && props.value.length && !inputRef.current.value) {
                        let removedValue = props.value[props.value.length - 1];
                        let newValue = props.value.slice(0, -1);
    ...
    const isSelected = (val) => {
        return props.value && props.value.some(v => ObjectUtils.equals(v, val));
    }
    ...
    const bindDocumentClickListener = () => {
        if (!documentClickListener.current) {
            documentClickListener.current = (event) => {
            ...
            document.addEventListener('click', documentClickListener.current);
        }
    }

    const unbindDocumentClickListener = () => {
        if (documentClickListener.current) {
            document.removeEventListener('click', documentClickListener.current);
            documentClickListener.current = null;
        }
    }

    const bindScrollListener = () => {
        if (!scrollHandler.current) {
            scrollHandler.current = new ConnectedOverlayScrollHandler(elementRef.current, () => {
                if (overlayVisible) {
                    hideOverlay();
                }
            });
    ...
    const isOutsideClicked = (event) => {
        return elementRef.current && (overlayRef.current && !overlayRef.current.contains(event.target)) && !isInputClicked(event);
    }

    const isInputClicked = (event) => {
        if (props.multiple)
            return event.target === multiContainerRef.current || multiContainerRef.current.contains(event.target);
        else
    ...
    const useSimpleAutoComplete = () => {
        const inputClassName = classNames('p-autocomplete-input', props.inputClassName, {
        ...
        return (
            <InputText ref={inputRef} id={props.inputId} type={props.type} name={props.name}
                ...
                aria-labelledby={props.ariaLabelledBy} className={inputClassName} style={props.inputStyle} autoComplete="off"
    ...
    const useMultipleAutoComplete = () => {
        const multiContainerClass = classNames('p-autocomplete-multiple-container p-component p-inputtext', {
        ...
        return (
            <ul ref={multiContainerRef} className={multiContainerClass} onContextMenu={props.onContextMenu} onMouseDown={props.onMouseDown}
    ...
    const input = props.multiple ? useMultipleAutoComplete() : useSimpleAutoComplete();
    const dropdown = props.dropdown ? useDropdown() : null;
